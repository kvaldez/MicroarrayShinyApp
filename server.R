     ...
     output$kegg=DT::renderDataTable(DT::datatable(
       {
         ...
         ## P value cutoff
         dat.i <- which(as.numeric(dat[,5]) < input$pval)
         dat.s <- dat[dat.i,]
         ## get gene list
         gene  <- as.character(dat.s[,9])
         if (input$Platform=="mst2") {
         xx=doGSEA(db="KEGG_mm", gene=gene, filter.num=0, fdr=T)
         
         } else {
           if (input$Platform=="h133p2") {
             xx=doGSEA(db="KEGG", gene=gene, filter.num=0, fdr=T)
           }
         };
         write.table(xx,file=paste(input$ProjectID,"_kegg_results_contrast_",input$NumContrasts,"_",input$pval,"_pvalue.txt",sep=""),sep="\t",row.names=F)
         xx
         ##
       }
     )
     ...
     output$go=DT::renderDataTable(DT::datatable(
       {
         ...
         if (input$Platform=="mst2") {
           yy=doGSEA(db="GO_mm", gene=gene, filter.num=0, fdr=T)
         } else {
           if (input$Platform=="h133p2") {
             yy=doGSEA(db="GO", gene=gene, filter.num=0, fdr=T)
           }
         };
         ##
         write.table(yy,file=paste(input$ProjectID,"_go_results_contrast_",input$NumContrasts,"_",input$pval,"_pvalue.txt",sep=""),sep="\t",row.names=F)
         yy
       }
     )
     )
